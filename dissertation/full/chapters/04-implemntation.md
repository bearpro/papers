# 3. Семантическая обратная связь и межмодельное выравнивание формальных моделей

Материал главы раскрывает часть алгоритмического обеспечения, связанную с подготовкой формальных моделей к сопоставлению. В отчёте перед научным советом этот блок относится к методам реализации системы обнаружения противоречий: канонизации имён, сопоставлению моделей и применению механизмов межмодельного выравнивания.

## 3.0. Связь с задачей поиска противоречий

Для поиска противоречий в нормативных документах требуется формальный способ записи нормативных положений. Формализм DDL-LTLf задаёт математический уровень такого представления. Инженерная проблема следующего уровня состоит в том, что формальные модели, полученные из разных документов или сгенерированные разными моделями, должны корректно сопоставляться друг с другом. Поэтому перед формальной проверкой противоречивости требуется этап межмодельного выравнивания.

Семантическая обратная связь рассматривается как способ упростить этот этап: стабилизировать имена типов, полей, ролей, единиц измерения и состояний ещё на этапе генерации или редактирования формальной модели.

## 3.1. Краткая характеристика результата semantic feedback

В работе исследуется применение LSP-подобной семантической обратной связи при генерации формальных C#-моделей, используемых как промежуточное представление нормативных и технических описаний. Проблема состоит в том, что независимо сгенерированные модели могут быть синтаксически корректными, но плохо сопоставимыми из-за различий в именовании сущностей, ролей, единиц измерения и связей. Новизна этого результата заключается в проверке семантического статического анализа как слоя нормализации моделей до межмодельного выравнивания. Эксперимент выполнен на шести документах, трёх генераторных моделях и трёх сценариях: контрольном, с инструкциями и с итеративной обратной связью. Получено 54 артефакта, 432 направленные пары и 298 745 кандидатов сопоставления.
...
## 3.2. Проблема сопоставимости формальных моделей
...
## 3.3. Постановка задачи и гипотеза
...
### 3.4.2. Генераторные модели и сценарии
...
### 3.5.1. Синтаксическое качество сгенерированного кода
...
### 3.5.2. Общий результат по всем методам
...
### 3.5.3. Конфигурация `valentine:coma_py`
...
### 3.5.4. Различия между документами
...
### 3.5.5. Сравнение алгоритмов выравнивания
...
### 3.5.6. Внутренняя оценка качества схем
...
Среднее значение `semantic_quality_index` для группы `control` составило 0.4532, для `guided` - 0.5070, `feedback` - 0.5565.
